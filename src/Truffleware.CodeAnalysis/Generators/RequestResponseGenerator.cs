...
{
    private static readonly string _abstractionsNamespace = typeof(IRequestSender<,>).Namespace
        ?? throw new InvalidOperationException("Assembly failed to load?");
    private static readonly string _requestHandlerAttributeFullName = typeof(RequestHandlerAttribute<,>).FullName
        ?? throw new InvalidOperationException("Assembly failed to load?");
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var handlerPipeline = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                fullyQualifiedMetadataName: _requestHandlerAttributeFullName,
            ...
            foreach (var requestResponse in handler.RequestResponses)
            {
                ...
                if (requestResponseDuplicates.Contains(requestResponse, _requestResponseComparer))
                {
                    var diagnostic = Diagnostic.Create(
                        ...
                        [ $"{nameof(RequestHandlerAttribute<,>)}<{requestResponse.RequestName}, {requestResponse.ResponseName}>", ]
                    );
    ...
    private static void CreateHandler(SourceProductionContext ctx, RequestHandler requestHandler)
    {
        ...
        var interfaceLines = requestHandler.RequestResponses
            .Select(r => $"{nameof(IRequestHandler<,>)}<{r.RequestName}, {r.ResponseName}>");
    ...
    private static void CreateServiceExtensionFile(SourceProductionContext ctx, ImmutableArray<RequestHandler> handlers)
    {
        ...
        var senderLines = handlers
            .SelectMany(h => h.RequestResponses)
            .Distinct()
            .Select(r => $"services.AddTransient<{nameof(IRequestSender<,>)}<{r.RequestName}, {r.ResponseName}>, {GetSenderClassName(r)}>();")
            .OrderBy(l => l);

        var handlerLines = handlers
            .SelectMany(h => h.RequestResponses, (handler, requestResponse) => (handler, requestResponse))
            .Select(x => $"services.AddTransient<{nameof(IRequestHandler<,>)}<{x.requestResponse.RequestName}, {x.requestResponse.ResponseName}>, {x.handler.ClassName}>();")
            .OrderBy(l => l);
